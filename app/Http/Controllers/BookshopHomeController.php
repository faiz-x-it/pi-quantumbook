    ...
    public function bookDetails($id)
{
    ...
    // Get similar items based on content-based filtering criteria
    $similarItems = Book::with('category')
        ->where('color', $book->color)
        ->where('ink_material', $book->ink_material)
        ->where('cloth_material', $book->cloth_material)
        ->where('id', '!=', $book->id) // Exclude the current book
        ->take(3) // Adjust the number of similar items as needed
        ->get();
        
    return view('public.book-details', compact('book', 'book_reviews', 'similarItems'));
}
